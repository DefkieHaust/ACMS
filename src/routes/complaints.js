...
router.post('/', validate(createComplaintSchema), async (req, res) => {
  try {
    ...
    if (!unit) return res.status(400).json({ error: 'No unit associated with your account' });
...
router.put('/:id/rate', async (req, res) => {
  try {
    ...
    const unit = await Unit.findOne({ residentUserId: req.user.userId });
    if (!unit) return res.status(403).json({ error: 'Only the resident who raised the complaint can rate' });
    ...
    if (!complaint) return res.status(404).json({ error: 'Complaint not found or not yet resolved' });
    res.json(complaint);
  } catch (err) {
